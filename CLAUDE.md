...
- **Offline / local-first**: while a plan is open, the Y.Doc is mirrored to per-plan IndexedDB via `y-indexeddb` (database name `ajmo:plan:<planId>`) so edits buffer locally when the WebSocket is down and replay via Hocuspocus's state-vector handshake on reconnect. Hocuspocus is still the only writer of `plans.yjs_state`; IndexedDB is purely a client-side buffer. Opt-in React Query results persist to IndexedDB under `ajmo:react-query-cache` (24h TTL, key `meta: { persist: true }`).
...
- Never construct the Hocuspocus provider before `IndexeddbPersistence.whenSynced` resolves (`createPlanDoc`'s `localLoaded`) — otherwise the sync handshake races the local hydration and offline edits land *after* the server's state, getting dropped.
- The Y.Doc + IndexedDB persistence are tied to `planId`; the Hocuspocus provider is tied to the auth token. Losing the token offline must NOT tear down the doc.
