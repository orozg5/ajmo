...
- Materializer offline-merge hardening (Phase 7f): (a) `coerce_integer_fields` rounds Yjs numeric fields back to `int` for `duration_minutes` and `sort_order` — pycrdt decodes all numbers as f64 and Postgres rejects `0.0` for `int` columns; (b) `text_or_string_to_str` flattens `Y.Text` notes to plain strings for the relational `notes` column while remaining backward-compatible with pre-Phase-7f docs that stored plain strings; (c) before upserting items, `drop_stale_cross_city_transport` (paired with `fetch_destination_ids`) discards cross-city transport rows whose source/destination `destination_id` no longer belongs to the plan — the defensive scrub for offline edits made while a peer deleted a destination.
- Seed (`services/collab/seed.py:build_seed_update`) wraps `plan_days.notes` and item `notes` in `Y.Text` instances before encoding, so cold-load docs match the live-edit Y.Text schema.
